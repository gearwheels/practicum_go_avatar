...
#    Пароли обязательны: дефолтных в чарте нет, и без них рендер падает.
...
Пароли попадают в Secret релиза. При `helm upgrade` их нужно передать снова,
и те же самые: Postgres и MinIO применяют пароль при инициализации тома, и с
новым значением поды к уже созданным данным не подключатся. Подсмотреть
текущие:

```bash
kubectl get secret gophprofile-secrets -n gophprofile \
...
Паролей по умолчанию в чарте нет ни в одном из файлов: известный всем дефолт,
забытый при выкате, — это скомпрометированные учётные данные в рабочем
окружении. Пароли либо передаются явно (`--set secrets.*Password=...`), либо
приходят из созданного вне чарта Secret (`secrets.existingSecret`, так сделано
в `values-prod.yaml`). Без того и другого `helm install` останавливается на
рендере с понятным сообщением, а не поднимает сервис с небезопасными
значениями. По той же причине `k8s/` содержит вместо паролей заглушки
`CHANGE_ME`.
...
- **Circuit breaker** стоит на каждой внешней зависимости: PostgreSQL, S3,
  ...
  Состояние видно в метрике `circuit_breaker_state{name}`. Источник времени у
  брейкера — внешняя зависимость (`resilience.Clock`), поэтому переход
  `open → half-open` проверяется в тестах без ожидания реального времени.
- **Ошибки наружу без внутренних деталей.** В ответах `4xx`, `500` и `503`
  клиент получает только обобщённое сообщение: текст оригинальной ошибки выдал
  бы имя таблицы, адрес зависимости или подстроку DSN. Настоящая причина
  уходит в лог сервера вместе с `trace_id`, по которому запись связывается с
  трейсом. То же относится к `/health` и `/livez`: они сообщают, что
  зависимость недоступна, но не чем именно она ответила.
